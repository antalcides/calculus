...
#pg:
	#install -d $(PGOUT)
	#cd $(PGOUT); \
	#xsltproc --xinclude --stringparam chunk.level 2 $(SRC)/extract-webwork.xsl $(SRC)/clm.xml
...
html:
	...
	cp -a $(CLMCSS) $(HTMLOUT)
	cd $(HTMLOUT); \
	xsltproc --xinclude  $(CLMXSL)/clm-html.xsl $(SRC)/clm.xml ;\
...
#webwork-server-tex:
	...
	$(MB)/script/mbx -v -c webwork-tex -f all  -d $(PDFOUT)/webwork-tex $(SRC)/clm.xml
...
latex:
	...
	-rm -R $(PDFOUT)/*.tex
	#cd $(PDFOUT); \
	xsltproc --xinclude   $(CLMXSL)/clm-print.xsl $(SRC)/clm.xml #; \
